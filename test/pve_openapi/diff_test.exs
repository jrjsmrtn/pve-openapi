...
defmodule PveOpenapi.DiffTest do
  ...
  describe "load_diff/2" do
    test "loads persisted diff JSON when available" do
      # specs/diffs/diff-8.3-8.4.json was generated by mix pve_openapi.diff
      diff = Diff.load_diff("8.3", "8.4")
      assert is_map(diff)
      assert diff["from"] == "8.3" || diff.from == "8.3"
    end

    test "falls back to runtime computation for non-persisted pairs" do
      # No persisted diff for non-consecutive pairs
      diff = Diff.load_diff("7.0", "9.0")
      assert is_map(diff)
      assert diff.from == "7.0"
      assert diff.to == "9.0"
    end
  end
end
